        ...
        if col in df.columns:
            if not df[col].between(min_val, max_val).all():
                if col == "weathercode":
                    ...
                    errors.append(f"{col} has invalid WMO codes: {invalid_codes}")
                else:
...
def check_duplicates(df: pd.DataFrame) -> list:
    ...
    if len(duplicate_times) > 0:
        errors.append(f"Found {len(duplicate_times)} duplicate (time, city) combinations")
...
def check_temporal_continuity(df: pd.DataFrame) -> list:
    ...
    if len(time_diffs) > 1:
        ...
        if len(gaps) > 0:
            warnings.append(
                f"Found {len(gaps)} time gaps larger than 1h 15m. Max gap: {gaps.max()}"
            )
...
def validate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if all_errors:
        ...
        raise ValueError(f"Data validation failed with {len(all_errors)} error(s): {error_details}")

    logger.info(f"Validation passed — {len(df)} rows, {null_counts.sum()} total nulls")
    return df
