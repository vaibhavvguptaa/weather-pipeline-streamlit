...
def load_data(db_path: str, last_modified: float) -> pd.DataFrame:
...
st.caption(
    "7-day hourly forecast data from [Open-Meteo API](https://open-meteo.com) "
    "— powered by an idempotent ETL pipeline"
)

# -- Load data -----------------------------------------------------------------
if not DB_PATH.exists():
    st.warning("No database found at `data/weather.db`. Run the pipeline first: `python main.py`")
    st.stop()

df = load_data(str(DB_PATH), DB_PATH.stat().st_mtime)
...
with col4:
    ...
    min_date = city_df["date"].min().strftime("%b %d")
    max_date = city_df["date"].max().strftime("%b %d")
